    ...
    <div class="main-card">
        ...
        <div class="page-toolbar">
            <div class="toolbar-left">
                ...
                <!-- Filter Button + Panel -->
                <div class="filter-btn-wrap" id="prodFilterWrap">
                    <button type="button" class="filter-btn" id="prodFilterBtn" onclick="prodFilters.togglePanel()">
                    ...
                    <!-- Filter Panel -->
                    <div class="fp-panel" id="prodFilterPanel">
                        <div class="fp-grid">
                            <!-- Search -->
                            <div class="fp-full">
                                <label class="fp-label" for="fp-search">Search</label>
                                <input type="text" id="fp-search" class="fp-input" placeholder="Name or SKU…">
                            </div>
                            <!-- Category -->
                            <div>
                                <label class="fp-label" for="fp-category">Category</label>
                                <select id="fp-category" class="fp-input">
                            ...
                            <!-- Status -->
                            <div>
                                <label class="fp-label" for="fp-status">Status</label>
                                <select id="fp-status" class="fp-input">
                                    <option value="">All</option>
                        ...
                        <div class="fp-actions">
                            <button type="button" class="btn-ghost" style="font-size:.78rem;padding:.3rem .7rem;" onclick="prodFilters.clearAll()">Clear</button>
                            <button type="button" class="btn btn-primary" style="font-size:.78rem;padding:.3rem .8rem;" onclick="prodFilters.applyAndClose()">Apply</button>
                        </div>
                    </div>
                </div>
            ...
            <div class="toolbar-right">
                <div class="record-pill" id="prodCount">
        ...
        <!-- Table -->
        <div class="tbl-wrap">
            <table class="dense-table" id="prodTable">
                <thead>
                    <tr>
                        <th>SKU</th>
                        <th>Product Name</th>
                ...
                <tbody>
                    <?php if (!empty($products)): ?>
                        ...
                        <tr
                            data-search="<?= htmlspecialchars(strtolower(($product['sku'] ?? '') . '|' . ($product['name'] ?? ''))) ?>"
                            data-category="<?= htmlspecialchars($product['category_id'] ?? '') ?>"
                            data-status="<?= $product['is_active'] ? '1' : '0' ?>"
                        >
                            <td><span class="mono-id"><?= htmlspecialchars($product['sku']) ?></span></td>
                        ...
                        <?php endforeach; ?>
                    <?php else: ?>
                        <tr id="prod-empty-initial">
                            <td colspan="7" class="empty-tbl">No products found. <a href="/products/create" class="view-btn" style="margin-left:.5rem;">Add Product</a></td>
                        </tr>
                    <?php endif; ?>
                    <!-- Empty state shown by JS when filters yield no results -->
                    <tr id="prod-empty-filtered" style="display:none;">
                        <td colspan="7" class="empty-tbl">No products match the current filters.</td>
                    </tr>
                </tbody>
            </table>
        </div>
...
(function () {
    ...
    const countEl   = document.getElementById('prodCount').querySelector('span');
    const tbody     = document.querySelector('#prodTable tbody');
    const emptyFiltered = document.getElementById('prod-empty-filtered');

    const fpSearch   = document.getElementById('fp-search');
    const fpCategory = document.getElementById('fp-category');
    ...
    document.addEventListener('click', function (e) {
        if (!document.getElementById('prodFilterWrap').contains(e.target)) {
            closePanel();
        }
    });
    ...
    function applyFilters() {
        ...
        state.category = fpCategory.value;
        ...
        rows.forEach(function (row) {
            ...
            const matchCategory = !state.category || row.dataset.category === state.category;
    ...
    function renderChips() {
        ...
        if (state.category) {
            const opt = fpCategory.querySelector('option[value="' + state.category + '"]');
            chipsEl.appendChild(makeChip('Cat: ' + (opt ? opt.textContent : state.category), 'category'));
        }
        if (state.status !== '') {
    ...
    function makeChip(label, filterId) {
        ...
        chip.className = 'chip';
        chip.innerHTML = label + ' <span class="chip-remove" onclick="prodFilters.clearFilter(\'' + filterId + '\')">&times;</span>';
        return chip;
    }

    function clearFilter(id) {
        if (id === 'search')   { state.search = '';   fpSearch.value   = ''; }
        if (id === 'category') { state.category = ''; fpCategory.value = ''; }
    ...
    function clearAll() {
        ...
        fpSearch.value = ''; fpCategory.value = ''; fpStatus.value = '';
        applyFilters();
    }

    function updateBtnState() {
        const hasActive = state.search || state.category || state.status !== '';
    ...
    // ── Expose globally ────────────────────────────────────────────────────
    window.prodFilters = { togglePanel, applyAndClose, clearFilter, clearAll };
